...
def _detect_patterns(bars: pd.DataFrame, cfg: dict, pivots: pd.DataFrame) -> pd.DataFrame:
    ...
    dedup_fn = globals().get("_deduplicate_patterns")
    if dedup_fn is None:
        logger.warning("pattern de-duplication helper is unavailable at runtime; continuing without deduplication")
        return combined
    return dedup_fn(combined, cfg)
