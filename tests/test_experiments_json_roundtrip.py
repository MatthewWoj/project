...
from patternfail.experiments.nesting import nesting_summary
...
def test_failure_and_nesting_helpers_accept_json_strings():
    patterns = pd.DataFrame(
        {
            "context_labels": ['{"session_bucket": "asia"}'],
            "outcome_labels": ['{"status": "failure", "time_to_exit": 3}'],
            "nested_in_pattern_id": [None],
        }
    )

    failure = failure_rate_by_context(patterns, "session_bucket")
    nesting = nesting_summary(patterns)

    assert failure.loc[0, "context"] == "asia"
    assert failure.loc[0, "failure_rate"] == 1.0
    assert nesting.loc[0, "failure_rate"] == 1.0
    assert nesting.loc[0, "mean_time_to_exit"] == 3.0
...
def test_run_experiments_layer_accepts_in_memory_json_strings(tmp_path: Path):
    dirs = ensure_run_dirs(str(tmp_path), "run")
    patterns = pd.DataFrame(
        {
            "pattern_id": ["p1"],
    ...
    _run_experiments_layer(cfg, dirs, all_patterns=patterns)
