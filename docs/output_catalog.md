...
You do not need a GUI parquet viewer. Use the bundled terminal helper:

```bash
python scripts/inspect_outputs.py --run-root data/outputs/<run_name> --list
```

Preview any table (parquet or csv fallback):
...
Expand JSON payload columns like `context_labels` and `outcome_labels` into ordinary columns:
...
For chart-style visual review of a single detected pattern, use:
...
python scripts/review_detection.py \
  ...
  --window-bars 120 \
  --show-outcome \
  --save data/outputs/<run_name>/figures/review_example.png
```
